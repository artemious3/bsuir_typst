...
#let template(first_page_number: false, doc) = {
  ...
  show enum: a => {
    let items = a
      ...
      .enumerate()
      .map(
        ((index, item)) => numbering(a.numbering, index + 1) + h(0.5em) + item.body + parbreak(),
      )
    parbreak() + items.join()
  }
  ...
  show list: a => {
    let items = a.children.map(
      item => a.marker + h(0.5em) + item.body + parbreak(),
    )
    parbreak() + items.join()
  }
  ...
  set footnote.entry(indent: 12.5mm, separator: line(length: 30% + 0pt, stroke: 1pt), clearance: 0em, gap: 1.15em)
